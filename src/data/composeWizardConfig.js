...
export const WIZARD_SECTIONS = [
  ...
  },
  {
    id: 'hls',
    title: 'HLS Storage',
    description: 'HLS segment storage, cleanup, and broadcast garbage collection',
    icon: '💾',
    collapsed: true,
    variables: [
      {
      ...
      {
        name: 'BROADCAST_GC_ENABLED',
        label: 'Broadcast GC Enabled',
        description: 'Automatically clean up orphaned broadcast HLS segments during programme transitions',
        type: FIELD_TYPES.BOOLEAN,
        default: true,
        showWhen: { field: 'NETWORK_BROADCAST_ENABLED', value: true },
        deploymentTypes: ['modular', 'aio', 'vpn', 'external-nginx', 'external-caddy'],
      },
      {
        name: 'BROADCAST_GC_INTERVAL',
        label: 'Broadcast GC Interval (seconds)',
        description: 'How often to scan for orphaned broadcast segments',
        type: FIELD_TYPES.NUMBER,
        default: '300',
        showWhen: { field: 'BROADCAST_GC_ENABLED', value: true },
        deploymentTypes: ['modular', 'aio', 'vpn', 'external-nginx', 'external-caddy'],
      },
      {
        name: 'BROADCAST_GC_AGE_THRESHOLD',
        label: 'Broadcast GC Age Threshold (seconds)',
        description: 'Remove stale inactive broadcast directories older than this value',
        type: FIELD_TYPES.NUMBER,
        default: '600',
        showWhen: { field: 'BROADCAST_GC_ENABLED', value: true },
        deploymentTypes: ['modular', 'aio', 'vpn', 'external-nginx', 'external-caddy'],
